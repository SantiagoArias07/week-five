...
const list = async (req, res) => {
  const rows = await db.prepare('SELECT * FROM planner_events WHERE user_id = ? ORDER BY date, hour').all(req.user.id);
...
const create = async (req, res) => {
  ...
  const row = await db.prepare('SELECT * FROM planner_events WHERE id = ?').get(info.lastInsertRowid);
  res.status(201).json(fmt(row));
};

const update = async (req, res) => {
  const existing = await db.prepare('SELECT * FROM planner_events WHERE id = ? AND user_id = ?').get(req.params.id, req.user.id);
  ...
  res.json(fmt(await db.prepare('SELECT * FROM planner_events WHERE id = ?').get(req.params.id)));
};

const remove = async (req, res) => {
  const { changes } = await db.prepare('DELETE FROM planner_events WHERE id = ? AND user_id = ?').run(req.params.id, req.user.id);
