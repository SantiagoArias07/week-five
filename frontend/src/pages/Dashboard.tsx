    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-0.5">
          You have{' '}
          <span className="font-medium text-indigo-600">{pendingTasks} pending tasks</span> today.
        </p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label="Total Tasks" value={totalTasks} icon={CheckSquare} iconColor="text-indigo-600" iconBg="bg-indigo-50" />
        <StatCard label="Completed" value={doneTasks} icon={TrendingUp} iconColor="text-green-600" iconBg="bg-green-50" trend={`${progressPercent}% done`} />
        <StatCard label="Subjects" value={subjects.length} icon={BookOpen} iconColor="text-amber-600" iconBg="bg-amber-50" />
        <StatCard label="Upcoming Exams" value={exams.length} icon={ClipboardList} iconColor="text-red-600" iconBg="bg-red-50" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-2.5">
              {todayTasks.map((task) => (
                <div key={task.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getStatusClasses(task.status)}`}>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          {upcomingExams.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-6">No upcoming exams</p>
          ) : (
            <div className="space-y-3">
              {upcomingExams.map((exam) => (
                <div key={exam.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                  <div
