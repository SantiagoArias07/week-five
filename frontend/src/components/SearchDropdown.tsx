...
export default function SearchDropdown({ query, onClose }: Props) {
  ...
  if (total === 0) {
    return (
      <div ref={ref} className="absolute left-0 top-10 w-72 bg-white rounded-xl border border-gray-200 shadow-xl z-50 p-4 text-center">
  ...
  return (
    <div ref={ref} className="absolute left-0 top-10 w-72 bg-white rounded-xl border border-gray-200 shadow-xl z-50 overflow-hidden">
      {matchedTasks.length > 0 && (
        <section>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-3 py-2 bg-gray-50">
            Tasks
          </p>
          {matchedTasks.map((t) => (
            <button
              key={t.id}
              onClick={() => go('/tasks')}
              className="w-full text-left flex items-center gap-2.5 px-3 py-2.5 hover:bg-gray-50 transition-colors border-b border-gray-50"
            >
